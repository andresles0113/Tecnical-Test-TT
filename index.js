process.stdin.resume();
 ...
 function whoIsOnCall (pStartDate, pDate, pPattern, pGroup) {
    if( pDate >= pStartDate ){
        let interval = Math.floor((pDate-pStartDate) / (1000 * 60 * 60 * 24));
        let PatternLength = pPattern.length;
        let GroupLength = pGroup.length;
        let patternCount = 0;
        let patternIterator = 0;
        let groupIterator = 0;
        while (patternCount < interval-1) { 
            if((patternCount + pPattern[patternIterator] )>interval){
                break;
            }           
        ...
        return "The pStartDate must be greater than pDate"
    }
